...
class SubCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->assertCategoryBelongsToStore($categoryIdForRule, $storeId);
    ...
    {
        $this->authorizeSubCategoryStore($subCategory);
    ...
    public function update(Request $request, SubCategory $subCategory)
    {
        ...
        $storeIdForUnique    = (int) $subCategory->store_location_id;
        $categoryIdForUnique = $request->input('category_id', $subCategory->category_id);

        $this->assertCategoryBelongsToStore($categoryIdForUnique, $storeIdForUnique);
    ...
    protected function authorizeSubCategoryStore(SubCategory $subCategory): void
    {
        $this->authorizeStoreAccess(
            request()->user(),
            (int) $subCategory->store_location_id
        );
    }

    protected function assertCategoryBelongsToStore($categoryId, int $storeId): void
    {
        $category = Category::find($categoryId);

        if (! $category || (int) $category->store_location_id !== $storeId) {
            abort(422, 'Category tidak valid untuk store ini.');
        }
    }
